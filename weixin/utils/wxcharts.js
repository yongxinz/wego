...
function getToolTipData2(categoriesData, seriesData, calPoints, index) {
    var textList = seriesData.map(function (item) {
        return {
            text: categoriesData + ': ' + item.data,
            color: item.color
        }
    })
    var validCalPoints = []
...
Charts.prototype.showToolTip = function (e) {
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area') {
        ...
        if (index > -1) {
            var categoriesData = this.opts.categories[index]
            var seriesData = getSeriesDataItem(this.opts.series, index);
            if (seriesData.length === 0) {
                drawCharts.call(this, opts.type, opts, this.config, this.context);
            } else {
                var _getToolTipData = getToolTipData2(categoriesData, seriesData, this.chartData.calPoints, index),
